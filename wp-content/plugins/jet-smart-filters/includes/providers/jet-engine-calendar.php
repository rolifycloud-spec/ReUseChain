...
if ( ! class_exists( 'Jet_Smart_Filters_Provider_Jet_Engine_Calendar' ) ) {
	...
	class Jet_Smart_Filters_Provider_Jet_Engine_Calendar extends Jet_Smart_Filters_Provider_Base {
		...
		/**
		 * Prepare settings before creating request signature.
		 */
		public function prepare_settings_for_signature( $data = array() ) {

			if ( ! is_array( $data ) ) {
				return $data;
			}

			unset( $data['custom_start_from'], $data['start_from_month'], $data['start_from_year'] );
