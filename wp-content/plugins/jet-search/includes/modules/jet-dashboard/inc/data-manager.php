...
class Data_Manager {
	...
	public function __construct() {
		$this->sys_messages = apply_filters( 'jet_dashboard_data_sys_messages', array(
	...
	public function get_service_action_list() {
		return array(
			...
			array(
				'label' => 'Sync License Data',
				'value' => 'sync-license-data',
			),
			array(
	...
	public function jet_dashboard_debug_action() {
		...
		switch ( $license_action ) {
			...
			case 'license-expire-check':
				...
				delete_site_transient( Dashboard::get_instance()->license_manager->license_remote_sync_key );
			...
			case 'sync-license-data':
				$synced_count = Dashboard::get_instance()->license_manager->license_remote_sync( true );

				wp_send_json(
					array(
						'status'  => 'success',
						'code'    => 'license_synced',
						'message' => sprintf( 'License data synced. Updated licenses: %d', $synced_count ),
						'data'    => array(
							'synced' => $synced_count,
						),
					)
