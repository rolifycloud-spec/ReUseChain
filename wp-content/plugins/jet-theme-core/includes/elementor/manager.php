...
class Manager {
	...
	public function print_templates() {
		...
		foreach ( glob( jet_theme_core()->plugin_path( 'templates/editor/*.php' ) ) as $file ) {
			...
			// The buffered plugin template is trusted markup for Elementor's editor.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			printf( '<script type="text/html" id="tmpl-jet-%1$s">%2$s</script>', esc_attr( sanitize_html_class( $name ) ), $template );
		}
	}
	...
	public function modify_template_library_column_type( $template_id ) {
		...
		if ( ! \Jet_Theme_Core\Utils::has_elementor() ) {
			...
			// The SVG icon is a static plugin asset.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			printf( '<div class="jet-template-library-column__template-not-editable">%1$s<span>%2$s</span></div>', $warning_icon, esc_html__( 'Template cannot be used, please install required content editor', 'jet-theme-core' ) );
		}
	}
	...
	public function editor_template_before() {
		...
		if ( isset( $editor['template_before'] ) ) {
			// The wrapper is trusted markup from the plugin configuration.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
