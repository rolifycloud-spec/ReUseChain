...
if ( ! class_exists( 'Jet_Theme_Core_Ajax_Handlers' ) ) {
	...
	class Jet_Theme_Core_Ajax_Handlers {
		...
		public function search_pages() {
			...
			// This read-only search is restricted to users who can edit posts.
		...
		public function search_posts() {
			...
			// phpcs:disable WordPress.Security.NonceVerification.Recommended
			$query     = isset( $_GET['q'] ) ? sanitize_text_field( wp_unslash( $_GET['q'] ) ) : '';
			$post_type = isset( $_GET['preview_post_type'] ) ? sanitize_key( wp_unslash( $_GET['preview_post_type'] ) ) : 'post';
			$ids       = isset( $_GET['ids'] ) ? wp_parse_id_list( wp_unslash( $_GET['ids'] ) ) : array();
			// phpcs:enable WordPress.Security.NonceVerification.Recommended
		...
		public function search_terms() {
			...
			$tax = array_filter( array_map( 'sanitize_key', explode( ',', $tax ) ), 'taxonomy_exists' );
