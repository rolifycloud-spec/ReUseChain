...
class Url_Param extends Base {
	...
	public function check( $arg = '' ) {
		...
		// Public request parameters are only compared and never mutate application state.
		...
		if ( strpos( $arg, '&' ) ) {
			...
			if ( ! empty( $param_list ) && is_array( $param_list ) ) {
				foreach ( $param_list as $key => $value ) {
					...
					if ( $param_pair ) {
						$params_data[ sanitize_key( $param_pair[0] ) ] = sanitize_text_field( $param_pair[1] );
					}
				}
			}
		} else {
		...
		// Preserve legacy GET, POST, and cookie matching while sanitizing compared values.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		foreach ( $request_data as $request_param => $request_value ) {
			...
			$request_param = sanitize_key( $request_param );
