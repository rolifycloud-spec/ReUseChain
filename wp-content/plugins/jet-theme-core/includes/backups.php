...
if ( ! class_exists( 'Jet_Theme_Core_Backups' ) ) {
	...
	class Jet_Theme_Core_Backups {
		...
		/**
		 * Return a validated path to a backup archive.
		 *
		 * @param string $file Backup filename.
		 *
		 * @return string|false
		 */
		public function get_backup_filepath( $file ) {
			...
			if ( ! $file || 'zip' !== strtolower( pathinfo( $file, PATHINFO_EXTENSION ) ) ) {
				return false;
			}

			$path     = trailingslashit( wp_normalize_path( $this->prepare_path( $this->path ) ) );
			$filepath = wp_normalize_path( $path . $file );
		...
		public function download_backup( $file ) {
			...
			$filepath = $this->get_backup_filepath( $file );

			if ( ! $filepath || ! $wp_filesystem->exists( $filepath ) ) {
		...
		public function readfile_chunked( $file, $retbytes = true ) {
			...
			while ( ! @feof( $handle ) ) {
				...
				// Raw backup bytes must not be HTML-escaped.
