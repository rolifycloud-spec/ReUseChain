...
class Utils {
	...
	/**
	 * Validate and decode an uploaded JSON file.
	 *
	 * @param array $file Uploaded file data.
	 *
	 * @return array|\WP_Error
	 */
	public static function read_uploaded_json( $file ) {

		if ( ! is_array( $file ) || UPLOAD_ERR_OK !== self::array_get( $file, 'error', UPLOAD_ERR_NO_FILE ) ) {
			return new \WP_Error( 'upload_error', __( 'File upload failed.', 'jet-theme-core' ) );
		}

		$file_name = sanitize_file_name( self::array_get( $file, 'name' ) );
		$tmp_name  = self::array_get( $file, 'tmp_name' );
		$file_size = absint( self::array_get( $file, 'size' ) );

		if ( 'json' !== strtolower( pathinfo( $file_name, PATHINFO_EXTENSION ) ) ) {
			return new \WP_Error( 'invalid_file_type', __( 'Format not allowed', 'jet-theme-core' ) );
		}

		if ( ! is_string( $tmp_name ) || ! is_uploaded_file( $tmp_name ) ) {
			return new \WP_Error( 'invalid_upload', __( 'Invalid uploaded file.', 'jet-theme-core' ) );
		}

		if ( ! $file_size || $file_size > wp_max_upload_size() ) {
			return new \WP_Error( 'invalid_file_size', __( 'Uploaded file size is not allowed.', 'jet-theme-core' ) );
		}

		$content = file_get_contents( $tmp_name );

		if ( false === $content ) {
			return new \WP_Error( 'file_read_error', __( 'Unable to read uploaded file.', 'jet-theme-core' ) );
		}

		$data = json_decode( $content, true );

		if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $data ) ) {
			return new \WP_Error( 'invalid_json', __( 'No data found in file', 'jet-theme-core' ) );
		}
