...
class Templates {
	...
	public function set_post_columns( $columns ) {
		...
		// This value only filters the current admin list table.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$current_type = isset( $_GET[ $this->type_tax ] ) ? sanitize_key( wp_unslash( $_GET[ $this->type_tax ] ) ) : '';

		if ( ! in_array( $current_type, $exclude_structures, true ) ) {
		...
		switch ( $column ) {
			...
			case 'type':
				...
				if ( $structure ) {
					...
					printf( '<a href="%1$s" class="jet-template-library-column__template-type"><span class="type-icon">%3$s</span><span>%2$s</span></a>', esc_url( $link ), esc_html( $structure->get_single_label() ), $icon );
			...
			case 'page-template':
				...
				// The manager returns trusted admin links and labels for page templates.
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			...
			case 'conditions':
				...
				// The conditions manager returns trusted admin markup.
				// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
				printf( '<div class="jet-template-library__template-conditions" data-template-id="%1$s" data-structure-type="%2$s">%3$s</div>',
					esc_attr( absint( $template_id ) ),
					esc_attr( sanitize_key( $structure->get_id() ) ),
					$template_verbose_conditions
				);
				// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped

				if ( $structure->has_conditions() ) {
					// The edit icon is selected from the plugin's static SVG map.
					// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
					printf(
						...
						esc_html__( 'Edit Conditions', 'jet-theme-core' )
					);
					// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped
				}
	...
	public function print_type_tabs( $edit_links ) {
		...
		// This value only filters the current admin list table.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$active_tab = isset( $_GET[ $this->type_tax ] ) ? sanitize_key( wp_unslash( $_GET[ $this->type_tax ] ) ) : 'all';
	...
	public function print_vue_templates() {
		...
		foreach ( glob( jet_theme_core()->plugin_path( 'templates/admin/templates-library/' )  . '*.php' ) as $file ) {
			...
			// The buffered plugin template is trusted Vue template markup.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			printf( '<script type="x-template" id="tmpl-jet-theme-core-%1$s">%2$s</script>', esc_attr( sanitize_html_class( $name ) ), $template );
		}
