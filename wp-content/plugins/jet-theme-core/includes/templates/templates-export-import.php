...
class Templates_Export_Import {
	...
	public function export_template_action_handler() {
		...
		if ( 'jet_theme_core_export_template' !== $action ) {
			return false;
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'jet-theme-core-template-export-nonce' ) ) {
			wp_send_json( [
				'status'  => 'error',
	...
	public function prepare_template_data_to_export( $template_ids ) {
		...
		foreach ( $template_ids as $template_id ) {
			...
			$editor_template_data = [
				'content'     => 'elementor' === $content_type ? [] : '',
				'meta_fields' => [],
			];
			...
			switch ( $content_type ) {
				case 'elementor':
					if ( \Jet_Theme_Core\Utils::has_elementor() ) {
						$exported_data = jet_theme_core()->elementor_manager->get_elementor_template_data_to_export( $template_id );
						$editor_template_data = is_array( $exported_data ) ? wp_parse_args( $exported_data, $editor_template_data ) : $editor_template_data;
					}
					break;
				default:
					$exported_data = $this->get_template_data_to_export( $template_id );
					$editor_template_data = is_array( $exported_data ) ? wp_parse_args( $exported_data, $editor_template_data ) : $editor_template_data;
					break;
			}
	...
	public function template_import_action() {
		...
		if ( ! wp_verify_nonce( $nonce, 'jet-theme-core-template-import-nonce' ) ) {
		...
		if ( empty( $content['templateList'] ) || ! is_array( $content['templateList'] ) ) {
			wp_send_json_error( [
	...
	public function create_imported_template( $import_data = false ) {
		...
		$required_fields = [ 'type', 'contentType' ];

		if ( ! is_array( $import_data ) || array_diff( $required_fields, array_keys( $import_data ) ) ) {
			return [
				'type'          => 'error',
		...
		if ( ! is_string( $import_data['type'] ) || ! is_string( $import_data['contentType'] ) ) {
			return [
				'type'        => 'error',
				'message'     => __( 'Invalid template data.', 'jet-theme-core' ),
				'template_id' => false,
			];
		}
		...
		$template_name = isset( $import_data['templateName'] ) && is_string( $import_data['templateName'] ) ? sanitize_text_field( $import_data['templateName'] ) : '';
		$type          = sanitize_key( $import_data['type'] );
		$content_type  = sanitize_key( $import_data['contentType'] );
		$content       = isset( $import_data['content'] ) ? $import_data['content'] : ( 'elementor' === $content_type ? [] : '' );
		$imported_meta_fields = isset( $import_data['metaFields'] ) && is_array( $import_data['metaFields'] ) ? $import_data['metaFields'] : [];

		// Older exports can contain null for an empty editor document.
		if ( null === $content ) {
			$content = 'elementor' === $content_type ? [] : '';
		}
		$meta_fields          = [
			'_jet_template_conditions'   => isset( $imported_meta_fields['_jet_template_conditions'] ) && is_array( $imported_meta_fields['_jet_template_conditions'] ) ? $imported_meta_fields['_jet_template_conditions'] : [],
			'_jet_template_content_type' => $content_type,
			'_jet_template_type'         => $type,
		];
		$meta_input    = [];
		...
		switch ( $content_type ) {
			case 'default':
				if ( ! is_string( $content ) ) {
					return [
						'type'        => 'error',
						'message'     => __( 'Invalid template content.', 'jet-theme-core' ),
						'template_id' => false,
					];
				}
			...
			case 'elementor':
				if ( ! is_array( $content ) ) {
					return [
						'type'        => 'error',
						'message'     => __( 'Invalid Elementor template content.', 'jet-theme-core' ),
						'template_id' => false,
					];
				}
			...
			default:
				return [
					'type'        => 'error',
					'message'     => __( 'Incorrect template content type.', 'jet-theme-core' ),
					'template_id' => false,
				];
		}
