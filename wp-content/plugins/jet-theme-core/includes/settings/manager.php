...
class Settings {
	...
	public function kava_theme_action() {
		...
		$data = ( ! empty( $_POST['data'] ) && is_array( $_POST['data'] ) ) ? map_deep( wp_unslash( $_POST['data'] ), 'sanitize_text_field' ) : false;
		...
		if ( ! wp_verify_nonce( $nonce, 'jet-theme-core-settings-nonce' ) ) {
			wp_send_json( array(
				'status'  => 'error',
				'code'    => false,
		...
		$action_type = isset( $data['actionType'] ) ? sanitize_key( $data['actionType'] ) : '';
	...
	public function backup_theme_action() {
		...
		$data = ( ! empty( $_REQUEST['data'] ) && is_array( $_REQUEST['data'] ) ) ? map_deep( wp_unslash( $_REQUEST['data'] ), 'sanitize_text_field' ) : false;
		...
		switch ( $action_type ) {
			...
			case 'delete':
				$delete = $this->get_backups_manager_instance()->delete_backup( $file );
