...
## 1.5.1
* Added: Optional quantity selector controls to the WooCommerce Shopping Cart widget.
* Fixed: Prevented Sticky Section jumps after applying JetSmartFilters filters.
* Fixed: Preserved the current admin page after completing the Jet Plugins Data Update.
* Updated: JetDashboard module to the latest version.

## 1.5.0.1
* Fixed: Sensitive data exposure in the Elementor template REST endpoint.

## 1.5.0
* Added: Custom Breadcrumb Items option for the Breadcrumbs widget.
* Added: Full Term Hierarchy option for the Breadcrumbs widget.
* Added: Role Based Redirect option for the Login Form widget.
* Added: Independent Mobile Menu styling options for the Navigation Menu widget.
* Fixed: JetSmartFilters pagination autoscroll conflict with JetBlocks sticky handler.
* Fixed: Deprecated dynamic property notice in JetBlocks settings.
* Updated: JetDashboard module to the latest version.

## 1.4.1
* Fixed: Restored smooth opening animation for the Hamburger Panel on the frontend.
* Fixed: Prevented JetSticky sections from shrinking after browser resizing/scaling.
* Fixed: Prevented Sticky Header from bouncing after filters are applied.
* Fixed: Hamburger Panel now closes correctly after anchor navigation triggered by the Navigation Menu on mobile.
* Fixed: Prevented Sticky Header from breaking when pagination is applied.
...
## 1.4.0
* Added: Schema.org BreadcrumbList micro-markup support for the Breadcrumbs widget.
* Added: Nav Menu “Open By Default” option with “Open Submenu on Item” setting to auto-open a specified submenu item in the default mobile menu layout.
* Added: Nav Menu enhancements including responsive Dropdown Background Color, Mobile Menu Dropdown Box Shadow control, configurable Mobile Overlay (with color), and a Custom “Start Showing Mobile Menu From” breakpoint (px).
