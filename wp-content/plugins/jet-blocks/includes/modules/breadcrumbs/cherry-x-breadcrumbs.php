...
if ( ! class_exists( 'CX_Breadcrumbs' ) ) {
	...
	class CX_Breadcrumbs {
		...
		/**
		 * Raw items data for JSON-LD schema.
		 *
		 * @since 1.0.7
		 * @var array
		 */
		public $raw_items_data = array();
		...
		public function __construct( $args = array() ) {
			...
			$defaults = array(
                ...
                'show_term_hierarchy' => false,
                'custom_breadcrumbs_items' => array(),
				'strings'            => array(),
				'date_labels'        => array(),
		...
		public function get_trail() {
			...
			if ( ! empty( $this->args['enable_json_ld'] ) ) {
				$json_ld = $this->get_json_ld_schema();
				if ( ! empty( $json_ld ) ) {
		 ...
		 * @since 1.0.7
		 *
		 * @return array JSON-LD schema array or empty array if no items.
		 */
		public function get_json_ld_schema() {

			if ( empty( $this->raw_items_data ) || ! is_array( $this->raw_items_data ) ) {
				return array();
			}
			...
			if ( count( $this->raw_items_data ) < 2 ) {
				return array();
			}
			...
			// Build schema from raw items data
			foreach ( $this->raw_items_data as $item_data ) {
				...
				$list_item = array(
					'@type' => 'ListItem',
					'position' => $position,
					'name' => html_entity_decode( $item_data['name'], ENT_QUOTES, 'UTF-8' ),
				);

				// Add URL only if it exists (not for the last/current item)
				if ( ! empty( $item_data['url'] ) ) {
					$list_item['item'] = array( '@id' => esc_url( $item_data['url'] ) );
				}
			...
			$schema = apply_filters( 'cx_breadcrumbs/json_ld_schema', $schema, $this->args );
		...
		public function build_trail() {
			...
			if ( is_front_page() ) {
                ...
                $this->add_custom_breadcrumbs_items( 'after_home' );
            ...
            $this->add_custom_breadcrumbs_items( 'before_current' );
		...
		public function _add_item( $format = 'link_format', $label = '', $url = '', $class = '', $prepend = false ) {
			...
			$raw_name = trim( wp_strip_all_tags( $label ) );

			// Store raw item data for JSON-LD schema
			$raw_item_data = array(
				'name' => $raw_name,
				'url'  => $url,
			);
			...
			if ( true === $prepend ) {
				array_unshift( $this->raw_items_data, $raw_item_data );
			} else {
				$this->raw_items_data[] = $raw_item_data;
			}
        ...
        /**
         * Add custom breadcrumb items by position.
         *
         * @param string $position Item position.
         */
        public function add_custom_breadcrumbs_items( $position = 'before_current' ) {

            if ( empty( $this->args['custom_breadcrumbs_items'] ) || ! is_array( $this->args['custom_breadcrumbs_items'] ) ) {
                return;
            }

            foreach ( $this->args['custom_breadcrumbs_items'] as $custom_item ) {

                $item_position = ! empty( $custom_item['position'] ) ? sanitize_key( $custom_item['position'] ) : 'before_current';

                if ( $position !== $item_position ) {
                    continue;
                }

                $prepared_item = $this->prepare_custom_breadcrumb_item( $custom_item );

                if ( empty( $prepared_item ) ) {
                    continue;
                }

                if ( 'before_current' === $position && ! empty( $this->items ) ) {
                    $insert_index = max( count( $this->items ) - 1, 0 );

                    array_splice( $this->items, $insert_index, 0, array( $prepared_item['item'] ) );
                    array_splice( $this->raw_items_data, $insert_index, 0, array( $prepared_item['raw'] ) );
                } else {
                    $this->items[]          = $prepared_item['item'];
                    $this->raw_items_data[] = $prepared_item['raw'];
                }
            }
        }

        /**
         * Prepare custom breadcrumb item.
         *
         * @param array $custom_item Custom item data.
         *
         * @return array|false
         */
        public function prepare_custom_breadcrumb_item( $custom_item = array() ) {

            if ( empty( $custom_item['label'] ) ) {
                return false;
            }

            $label = wp_kses_post( $custom_item['label'] );
            $label = $this->prepare_label( $label );

            if ( empty( $label ) ) {
                return false;
            }

            $url = ! empty( $custom_item['url'] ) ? trim( $custom_item['url'] ) : '';
            ...
            return array(
                'item' => $result,
                'raw'  => array(
                    'name' => trim( wp_strip_all_tags( $label ) ),
                    'url'  => $url,
                ),
            );
        }
        ...
        /**
         * Get the deepest term from a terms list.
         *
         ...
         */
        public function get_deepest_term( $terms, $taxonomy ) {
		...
		public function add_post_terms( $post_id ) {
			...
			if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
				...
				if ( is_array( $post_terms ) && isset( $post_terms[0] ) && is_object( $post_terms[0] ) ) {
                    ...
                    if ( ! empty( $this->args['show_term_hierarchy'] ) && is_taxonomy_hierarchical( $taxonomy ) ) {
                        $deepest_term = $this->get_deepest_term( $post_terms, $taxonomy );
