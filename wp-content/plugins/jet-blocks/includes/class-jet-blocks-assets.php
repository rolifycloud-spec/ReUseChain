...
if ( ! class_exists( 'Jet_Blocks_Assets' ) ) {
	...
	class Jet_Blocks_Assets {
		...
		public function enqueue_styles() {
			...
			$version = get_option( 'jet-blocks-version' );

			if ( ! $version || version_compare( $version, jet_blocks()->get_version(), '<' ) ) {
				$frontend_file->update();
				update_option( 'jet-blocks-version', jet_blocks()->get_version(), true );
			}
