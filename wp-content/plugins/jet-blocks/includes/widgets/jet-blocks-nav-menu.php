...
class Jet_Blocks_Nav_Menu extends Jet_Blocks_Base {
	...
	protected function register_controls() {
		...
		if ( \Elementor\Plugin::$instance->breakpoints && method_exists( \Elementor\Plugin::$instance->breakpoints, 'get_active_breakpoints')) {
			...
			$breakpoints_list['custom'] = esc_html__( 'Custom', 'jet-blocks' );
		} else {
			$breakpoints_list = array(
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Start Showing Mobile Menu From', 'jet-blocks' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'mobile',
		...
		$this->add_control(
			'mobile_trigger_custom_breakpoint',
			[
				'label'     => esc_html__( 'Mobile Menu Starts From (px)', 'jet-blocks' ),
		...
		$this->add_control(
			'mobile_open_by_default',
			[
				'label'     => esc_html__( 'Open By Default', 'jet-blocks' ),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => '',
				'separator' => 'before',
				'condition' => [
		...
		$this->add_control(
			'mobile_open_submenu_item',
			[
				'label'       => esc_html__( 'Open Submenu on Item', 'jet-blocks' ),
				'type'        => Controls_Manager::NUMBER,
				'default'     => '',
				'min'         => 1,
				'step'        => 1,
				'description' => esc_html__(
					'Specify the global menu item number (position in the whole menu, not only items with submenu). If the specified item has no submenu, nothing will be opened. Leave empty to open only the top-level mobile menu.',
					'jet-blocks'
				),
				'condition'   => [
					'mobile_trigger_visible'  => 'yes',
					'mobile_menu_layout'      => 'default',
					'mobile_open_by_default'  => 'yes',
				],
			]
		);

		$this->__add_control(
			'mobile_overlay_enable',
			[
				'label'     => esc_html__( 'Enable Overlay', 'jet-blocks' ),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => '',
				'separator' => 'before',
				'condition' => [
		...
		$this->__add_control(
			...
			[
				'label'     => esc_html__( 'Overlay Background Color', 'jet-blocks' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}}' => '--jet-nav-mobile-overlay-bg: {{VALUE}};',
				],
				'condition' => [
					'mobile_overlay_enable' => 'yes',
					'mobile_trigger_visible' => 'yes',
				],
			],
			25
		...
		$this->__add_control(
			...
			array(
				...
				'type'   => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}}' => '--jet-nav-dropdown-bg: {{VALUE}};',
				),
			),
			25
		);
		...
		$this->__add_control(
			...
			array(
				...
				'selectors' => array(
                    ...
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav > .menu-item > .menu-item-link-top' => 'background-color: {{VALUE}};',
				),
			),
			25
		);
        ...
        $this->__add_control(
            ...
            array(
                ...
                'type'  => Controls_Manager::COLOR,
                'selectors' => array(
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav .menu-item > .menu-item-link' => 'color: {{VALUE}};',
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav .menu-item > .menu-item-link .jet-nav-link-text' => 'color: {{VALUE}};',
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav .menu-item > .menu-item-link .jet-nav-arrow' => 'color: {{VALUE}};',
                ),
            ),
            25
        );
		...
		$this->__add_control(
			'mobile_menu_text_bg_color',
			array(
				'label' => esc_html__( 'Text Background Color', 'jet-blocks' ),
				'type'  => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav .menu-item > .menu-item-link .jet-nav-link-text' => 'background-color: {{VALUE}};',
				),
			),
			75
		);
		...
		$this->__add_control(
			'mobile_dropdown_bg_color',
			[
				'label' => esc_html__( 'Dropdown Background Color', 'jet-blocks' ),
				'type'  => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}}' => '--jet-nav-mobile-dropdown-bg: {{VALUE}};',
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav__sub' => 'background-color: {{VALUE}};',
                    '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav__sub .menu-item-link-sub' => 'background-color: {{VALUE}};',
				],
			],
			25
		);
		...
		$this->__add_group_control(
			Group_Control_Box_Shadow::get_type(),
			[
				'label'    => esc_html__( 'Dropdown Box Shadow', 'jet-blocks' ),
				'name'     => 'mobile_dropdown_box_shadow',
				'selector' => '{{WRAPPER}} .jet-nav-wrap.jet-mobile-menu-active .jet-nav__sub',
			],
			75
		);
	...
	protected function render() {
		...
		$custom_breakpoint = ! empty( $settings['mobile_trigger_custom_breakpoint'] )
			? absint( $settings['mobile_trigger_custom_breakpoint'] )
			: 0;
		...
		if ( $trigger_visible ) {
			...
			if ( 'custom' === $mobile_layout_device && $custom_breakpoint ) {
				$this->add_render_attribute( 'nav-wrapper', 'data-mobile-trigger-custom', absint( $custom_breakpoint ) );
			}

			$open_by_default = ! empty( $settings['mobile_open_by_default'] ) && filter_var( $settings['mobile_open_by_default'], FILTER_VALIDATE_BOOLEAN );

			if ( $open_by_default && isset( $settings['mobile_menu_layout'] ) && 'default' === $settings['mobile_menu_layout'] ) {
				$this->add_render_attribute( 'nav-wrapper', 'data-mobile-open-default', '1' );

				if ( ! empty( $settings['mobile_open_submenu_item'] ) ) {
					$this->add_render_attribute(
						'nav-wrapper',
						'data-mobile-open-submenu-item',
						absint( $settings['mobile_open_submenu_item'] )
					);
				}
			}
		}
		...
		$overlay_enabled = ! empty( $settings['mobile_overlay_enable'] ) && filter_var( $settings['mobile_overlay_enable'], FILTER_VALIDATE_BOOLEAN );
		echo '<nav ' . $this->get_render_attribute_string( 'nav-wrapper' ) . '>';
			if ( $trigger_visible ) {
				if ( $overlay_enabled ) {
					echo '<div class="jet-mobile-menu__overlay" aria-hidden="true"></div>';
				}
