...
class Jet_Blocks_Breadcrumbs extends Jet_Blocks_Base {
	...
	protected function register_controls() {
        ...
        $this->add_control(
            'show_term_hierarchy',
            array(
                'label'        => esc_html__( 'Show Full Term Hierarchy', 'jet-blocks' ),
                'type'         => Controls_Manager::SWITCHER,
                'default'      => '',
                'description'  => esc_html__( 'Show parent terms for hierarchical taxonomies.', 'jet-blocks' ),
            )
        );
		...
		$this->add_control(
			'enable_json_ld',
			array(
				'label'   => esc_html__( 'Enable JSON-LD breadcrumbs', 'jet-blocks' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			)
		);
        ...
        $custom_breadcrumbs_repeater = new Repeater();

        $custom_breadcrumbs_repeater->add_control(
            'label',
            array(
                'label'       => esc_html__( 'Label', 'jet-blocks' ),
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( 'Custom Item', 'jet-blocks' ),
                'label_block' => true,
            )
        );
        ...
        $custom_breadcrumbs_repeater->add_control(
            'url',
            array(
                'label'       => esc_html__( 'URL', 'jet-blocks' ),
                'type'        => Controls_Manager::TEXT,
                'default'     => '',
                'placeholder' => esc_html__( 'https://example.com/page/ or /page/', 'jet-blocks' ),
                'label_block' => true,
            )
        );
        ...
        $custom_breadcrumbs_repeater->add_control(
            ...
            array(
                ...
                'type'    => Controls_Manager::SELECT,
                'default' => 'before_current',
                'options' => array(
                    'after_home'     => esc_html__( 'After Home', 'jet-blocks' ),
                    'before_current' => esc_html__( 'Before Current Item', 'jet-blocks' ),
                ),
            )
        );

        $this->add_control(
            'custom_breadcrumbs_items',
            array(
                'label'       => esc_html__( 'Custom Breadcrumb Items', 'jet-blocks' ),
                'type'        => Controls_Manager::REPEATER,
                'fields'      => $custom_breadcrumbs_repeater->get_controls(),
                'title_field' => '{{{ label }}}',
                'separator'   => 'before',
                'prevent_empty' => false,
                'classes' => 'jet-breadcrumbs-custom-items-control',
                'description' => esc_html__( 'Add custom breadcrumb items into the breadcrumb path.', 'jet-blocks' ),
            )
        );
	...
	protected function render() {
		...
		$enable_json_ld           = isset( $settings['enable_json_ld'] )
			? filter_var( $settings['enable_json_ld'], FILTER_VALIDATE_BOOLEAN )
			: false;
		...
		$args = array(
            ...
            'custom_breadcrumbs_items' => ! empty( $settings['custom_breadcrumbs_items'] ) && is_array( $settings['custom_breadcrumbs_items'] )
                ? $settings['custom_breadcrumbs_items']
                : array(),
            ...
            'show_term_hierarchy' => ! empty( $settings['show_term_hierarchy'] )
                ? filter_var( $settings['show_term_hierarchy'], FILTER_VALIDATE_BOOLEAN )
                : false,
			'strings' => array(
