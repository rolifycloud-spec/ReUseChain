...
class Jet_Blocks_Login extends Jet_Blocks_Base {
	protected $use_role_based_redirect = false;

	protected $role_based_redirects = array();
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'role_based' => esc_html__( 'Role Based Redirect', 'jet-blocks' ),
				),
			)
		);
		...
		$role_redirect_repeater = new Repeater();

		$role_redirect_repeater->add_control(
			'role',
			array(
				'label'   => esc_html__( 'User Role', 'jet-blocks' ),
		...
		$role_redirect_repeater->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
				'placeholder' => esc_html__( '/my-account/', 'jet-blocks' ),
				'description' => esc_html__( 'Enter a full URL or a relative path. Examples: /my-account/, https://example.com/account/.', 'jet-blocks' ),
				'label_block' => true,
				'dynamic'     => array(
					'active' => true,
				),
		...
		$this->add_control(
			...
			array(
				'label'         => esc_html__( 'Role Based Redirects', 'jet-blocks' ),
				'type'          => Controls_Manager::REPEATER,
				'fields'        => $role_redirect_repeater->get_controls(),
				'title_field'   => '{{{ role }}}',
				'prevent_empty' => false,
				'condition'     => array(
					'login_redirect' => 'role_based',
				),
			)
		);
	...
	protected function render() {
		...
		$this->use_role_based_redirect = 'role_based' === $settings['login_redirect'];
		$this->role_based_redirects    = ! empty( $settings['role_based_redirects'] ) && is_array( $settings['role_based_redirects'] )
			? $settings['role_based_redirects']
			: array();
	...
	public function add_login_fields( $content ) {
		...
		if ( $this->use_role_based_redirect && ! empty( $this->role_based_redirects ) ) {
			$content .= '<input type="hidden" name="jet_login_redirect_type" value="role_based">';
			$content .= '<input type="hidden" name="jet_login_role_based_redirects" value="' . esc_attr( wp_json_encode( $this->role_based_redirects ) ) . '">';
			$content .= wp_nonce_field( 'jet_login_role_based_redirect', 'jet_login_role_based_redirect_nonce', true, false );
		}
		return $content;
	}
	...
	public function get_user_roles_options() {
		...
		$options = array(
			'' => esc_html__( 'Select role', 'jet-blocks' ),
		);
