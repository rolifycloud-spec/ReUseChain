...
namespace {
    ...
    if (!\class_exists('JFB_License_Manager')) {
        ...
        class JFB_License_Manager
        {
            ...
            public function license_action()
            {
                ...
                if (!$responce_data['success']) {
                    if ('invalid' === $responce_data['license']) {
                        \wp_send_json(array('success' => \false, 'message' => \__('Invalid license key', 'jet-form-builder'), 'data' => array()));
                    }
                    if ('failed' === $responce_data['license']) {
                        ...
                        \wp_send_json(array('success' => \true, 'message' => \__('The license for this site is already activated', 'jet-form-builder'), 'data' => array()));
                    }
                    \wp_send_json(array('success' => \false, 'message' => \__('Server error. Please, try again later', 'jet-form-builder'), 'data' => array()));
                }
                switch ($license_action) {
                    case 'activate_license':
                        $this->add_license_data($license_key, $responce_data);
                        $responce_data['license_key'] = $license_key;
                        \wp_send_json(array('success' => \true, 'message' => \__('The license has been successfully activated', 'jet-form-builder'), 'data' => $responce_data));
                        break;
                    case 'deactivate_license':
                        ...
                        \update_option($this->license_data_key, $license_list);
                        \wp_send_json(array('success' => \true, 'message' => \__('The license has been successfully deactivated', 'jet-form-builder'), 'data' => $responce_data));
                        break;
                }
                \wp_send_json(array('success' => \false, 'message' => \__('Server error. Please, try again later', 'jet-form-builder'), 'data' => array()));
            }
            ...
            {
                $query_url = \add_query_arg(array('edd_action' => $action, 'item_id' => 9, 'license' => $license, 'url' => \urlencode($this->get_site_url())), $this->api_url);
                $response = \wp_remote_get($query_url, array('timeout' => 60));
            ...
            {
                if ('0000-00-00 00:00:00' === $expire_date || '1000-01-01 00:00:00' === $expire_date || 'lifetime' === $expire_date) {
                    return \false;
                }
                $current_time = \time();
            ...
            public function package_url($plugin_slug = \false)
            {
                ...
                return \add_query_arg(array('ct_api_action' => 'get_plugin', 'license' => $license_key, 'slug' => $plugin_slug, 'url' => \urlencode($this->get_site_url())), $this->api_url);
            }
            /**
            ...
            public function addon_install_action()
            {
                ...
                \wp_send_json(array('success' => \true, 'message' => \__('Success', 'jet-form-builder'), 'data' => array()));
            }
            ...
            public function install_plugin($plugin_file, $plugin_url = \false)
            {
                ...
                if (!\current_user_can('install_plugins')) {
                    \wp_send_json(array('success' => \false, 'message' => \__('Sorry, you are not allowed to install plugins on this site.', 'jet-form-builder'), 'data' => array()));
                }
                if (!$plugin_url) {
                ...
                if (\is_wp_error($result)) {
                    ...
                    \wp_send_json(array('success' => \false, 'message' => $result->get_error_message(), 'data' => array()));
                } elseif (\is_wp_error($skin->result)) {
                    $status['errorCode'] = $skin->result->get_error_code();
                    $status['errorMessage'] = $skin->result->get_error_message();
                    \wp_send_json(array('success' => \false, 'message' => $skin->result->get_error_message(), 'data' => array()));
                } elseif ($skin->get_errors()->get_error_code()) {
                    $status['errorMessage'] = $skin->get_error_messages();
                    \wp_send_json(array('success' => \false, 'message' => $skin->get_error_messages(), 'data' => array()));
                    ...
                    \wp_send_json(array('success' => \false, 'message' => $status['errorMessage'], 'data' => array()));
                }
                \wp_send_json(array('success' => \true, 'message' => \__('The addon has been Installed', 'jet-form-builder'), 'data' => \Jet_Form_Builder\Plugin::instance()->addons_manager->get_addon_data($plugin_file)));
            }
            /**
             * @param $plugin_file
             */
            public function update_plugin($plugin_file)
            {
                ...
                $slug = \dirname($plugin);
                $status = array('update' => 'plugin', 'slug' => $slug, 'oldVersion' => '', 'newVersion' => '');
                if (!\current_user_can('update_plugins') || 0 !== \validate_file($plugin)) {
                    \wp_send_json(array('success' => \false, 'message' => \__('Sorry, you are not allowed to update plugins on this site.', 'jet-form-builder'), 'data' => array()));
                }
                include_once \ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
                \wp_update_plugins();
                $skin = new \WP_Ajax_Upgrader_Skin();
                ...
                if (\is_wp_error($skin->result)) {
                    \wp_send_json(array('success' => \false, 'message' => $skin->result->get_error_message(), 'data' => array(), 'debug' => $upgrade_messages));
                } elseif ($skin->get_errors()->get_error_code()) {
                    \wp_send_json(array('success' => \false, 'message' => $skin->get_error_messages(), 'data' => array(), 'debug' => $upgrade_messages));
                     ...
                     */
                    if (\true === $plugin_update_data) {
                        \wp_send_json(array('success' => \false, 'message' => \__('Addon update failed.', 'jet-form-builder'), 'data' => array(), 'debug' => $upgrade_messages));
                    }
                    \wp_send_json(array('success' => \true, 'message' => \__('The addon has been updated', 'jet-form-builder'), 'data' => \Jet_Form_Builder\Plugin::instance()->addons_manager->get_addon_data($plugin_file)));
                } elseif (\false === $result) {
                    global $wp_filesystem;
                    ...
                    \wp_send_json(array('success' => \false, 'message' => $errorMessage, 'data' => array()));
                }
                \wp_send_json(array('success' => \false, 'message' => \__('Plugin update failed.', 'jet-form-builder'), 'data' => array()));
            }
            /**
             * @param $data
             *
            ...
            {
                if (!\wp_verify_nonce(\sanitize_key($_POST['nonce'] ?? ''), \Jet_Form_Builder\Addons\Manager::NONCE_ACTION)) {
                    \wp_send_json(array('success' => \false, 'message' => \__('Server error. Please, try again later', 'jet-form-builder')));
                }
