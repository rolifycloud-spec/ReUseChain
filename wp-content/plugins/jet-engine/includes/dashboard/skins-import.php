	...
	class Jet_Engine_Skins_Import {
		...
		public function import_queries( $queries = array() ) {
			...
			foreach ( $queries as $query ) {
				...
				$query = jet_engine_prepare_serialized_row(
					$query,
					array(
						'labels'      => array(),
						'args'        => array(),
		...
		public function import_options_pages( $options_pages ) {
			...
			foreach ( $options_pages as $page ) {
				...
				$page = jet_engine_prepare_serialized_row(
					$page,
					array(
						'labels'      => array(),
						'args'        => array(),
			...
			foreach ( $post_types as $post_type ) {
				...
				switch ( $action ) {
					...
					case 'skip':
						...
						$post_type_labels = jet_engine_safe_unserialize_array( $post_type['labels'], 'skins_import/post_type_labels' );
						$this->log['post_types']['items'][] = ( $post_type_labels['name'] ?? '' ) . ' - skipped';
				...
				$post_type = jet_engine_prepare_serialized_row(
					$post_type,
					array(
						'labels'      => array(),
						'args'        => array(),
			...
			foreach ( $taxonomies as $tax ) {
				...
				switch ( $action ) {
					...
					case 'skip':
						...
						$tax_labels = jet_engine_safe_unserialize_array( $tax['labels'], 'skins_import/taxonomy_labels' );
						$this->log['taxonomies']['items'][] = ( $tax_labels['name'] ?? '' ) . ' - skipped';
				...
				$tax = jet_engine_prepare_serialized_row(
					$tax,
					array(
						'object_type' => array(),
						'labels'      => array(),
