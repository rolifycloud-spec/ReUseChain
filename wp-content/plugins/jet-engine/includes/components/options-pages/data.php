...
if ( ! class_exists( 'Jet_Engine_Options_Page_Sanitize' )
	&& file_exists( __DIR__ . '/sanitize.php' )
) {
...
if ( ! class_exists( 'Jet_Engine_Options_Data' ) ) {
	...
	class Jet_Engine_Options_Data extends Jet_Engine_Base_Data {
		...
		/**
		 * Sanitize Options Pages fields and normalize their write policy.
		 *
		 * @param array $meta_fields Fields received from the editor or API.
		 * @return array
		 */
		public function sanitize_meta_fields( $meta_fields ) {
			$meta_fields = parent::sanitize_meta_fields( $meta_fields );

			return $this->normalize_fields_sanitize( $meta_fields );
		}

		/**
		 * Apply the compatibility-safe sanitizer default to fields recursively.
		 *
		 * @param array $fields Fields to normalize.
		 * @return array
		 */
		public function normalize_fields_sanitize( $fields ) {
			foreach ( $fields as $key => $field ) {
				if ( ! is_array( $field ) ) {
					continue;
				}

				$field['sanitize'] = Jet_Engine_Options_Page_Sanitize::normalize(
					isset( $field['sanitize'] ) ? $field['sanitize'] : 'default'
				);

				if ( ! empty( $field['repeater-fields'] ) && is_array( $field['repeater-fields'] ) ) {
					$field['repeater-fields'] = $this->normalize_fields_sanitize( $field['repeater-fields'] );
				}
		...
		public function filter_item_for_edit( $item ) {
			...
			if ( ! empty( $item['meta_fields'] ) ) {
				...
				$fields = $this->normalize_fields_sanitize( $fields );
