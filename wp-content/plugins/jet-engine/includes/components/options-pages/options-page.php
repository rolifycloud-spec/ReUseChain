...
if ( ! class_exists( 'Jet_Engine_Options_Page_Sanitize' ) ) {
	require jet_engine()->options_pages->component_path( 'sanitize.php' );
}
...
if ( ! class_exists( 'Jet_Engine_Options_Page_Factory' ) ) {
	...
	class Jet_Engine_Options_Page_Factory extends Jet_Engine_CPT_Meta {
		...
		public function maybe_apply_sanitize_callback( $value, $field ) {
			...
			if ( is_array( $value ) && 'repeater' === $field['type'] && ! empty( $field['fields'] ) ) {
				foreach ( $value as $item_id => $item ) {
					...
					foreach ( $item as $sub_item_id => $sub_item_value ) {
						if ( ! isset( $field['fields'][ $sub_item_id ] ) ) {
							continue;
						}
				...
				// Preserve existing parent repeater callbacks after child values are normalized.
			}
			...
			$sanitize = Jet_Engine_Options_Page_Sanitize::normalize(
				isset( $field['sanitize'] ) ? $field['sanitize'] : 'default'
			);

			if ( 'default' !== $sanitize && is_string( $value ) ) {
				return Jet_Engine_Options_Page_Sanitize::sanitize_value( $value, $sanitize );
			}
