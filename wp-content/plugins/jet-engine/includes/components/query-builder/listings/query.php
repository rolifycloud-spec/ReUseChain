...
class Query {
	...
	public function query_items( $items, $settings, $widget ) {
		...
		$query_context = array(
			'type'       => jet_engine()->listings->is_listing_ajax() ? 'listing_ajax' : 'listing',
			'query_id'   => $query_id,
			'listing_id' => $listing_id,
			'settings'   => $settings,
		);

		$query = Query_Manager::instance()->get_query_by_id_for_context( $query_id, $query_context );
	...
	public function maybe_setup_load_more_prop( $query ) {
		...
		if ( ! empty( $_REQUEST['query']['filtered_query'] ) ) {
			$filtered_query = $this->sanitize_filtered_query_args( $_REQUEST['query']['filtered_query'], $query );
	...
	public function maybe_add_load_more_query_args( $request, $query, $settings ) {
		...
		if ( $is_custom_query && ! empty( $settings['custom_query_id'] ) ) {
			$request['custom_query_id'] = absint( $settings['custom_query_id'] );
		}

		return $request;
	}

	protected function sanitize_filtered_query_args( $filtered_query, $query = null ) {
