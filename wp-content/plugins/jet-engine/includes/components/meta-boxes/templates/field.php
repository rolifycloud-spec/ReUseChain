<div>
	...
	<cx-vui-select
		...
		></cx-vui-select>
		<cx-vui-select
			v-if="showSanitize && isSanitizeSupportedField( field )"
			label="<?php _e( 'Sanitize value', 'jet-engine' ); // phpcs:ignore ?>"
			description="<?php _e( 'Choose how this value is sanitized before it is stored. Raw stores untrusted markup unchanged and is not suitable for public forms. Output must still be escaped for its context.', 'jet-engine' ); // phpcs:ignore ?>"
			:wrapper-css="[ 'equalwidth' ]"
			size="fullwidth"
			:options-list="getSanitizeOptions( field.type )"
			:value="field.sanitize || 'default'"
			@input="setFieldProp( 'sanitize', $event )"
		></cx-vui-select>
		<cx-vui-select
		...
		<div class="cx-vui-inner-panel">
			...
			>
				...
				>
					...
					></cx-vui-select>
					<cx-vui-select
						v-if="showSanitize && isSanitizeSupportedField( field['repeater-fields'][ rFieldIndex ] )"
						label="<?php _e( 'Sanitize value', 'jet-engine' ); // phpcs:ignore ?>"
						description="<?php _e( 'Choose how this value is sanitized before it is stored. Raw stores untrusted markup unchanged and is not suitable for public forms. Output must still be escaped for its context.', 'jet-engine' ); // phpcs:ignore ?>"
						:wrapper-css="[ 'equalwidth' ]"
						size="fullwidth"
						:options-list="getSanitizeOptions( field['repeater-fields'][ rFieldIndex ].type )"
						:value="field['repeater-fields'][ rFieldIndex ].sanitize || 'default'"
						@input="setRepeaterFieldProp( rFieldIndex, 'sanitize', $event )"
					></cx-vui-select>
					<cx-vui-select
						label="<?php _e( 'Source', 'jet-engine' ); // phpcs:ignore ?>"
						description="<?php _e( 'Select source to get field options from', 'jet-engine' ); // phpcs:ignore ?>"
