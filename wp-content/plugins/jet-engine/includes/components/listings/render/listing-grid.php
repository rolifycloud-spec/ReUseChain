...
if ( ! class_exists( 'Jet_Engine_Render_Listing_Grid' ) ) {
	...
	class Jet_Engine_Render_Listing_Grid extends Jet_Engine_Render_Base {
		...
		public function print_lazy_load_wrapper( $settings ) {
			...
			$custom_query = jet_engine()->listings->ajax_handlers->get_lazy_load_custom_query( $settings );

			if ( ! empty( $custom_query ) ) {
				$options['query'] = $custom_query;
			}
		...
		public function get_nav_settings( $settings ) {
			...
			if ( ! empty( $result['query'] ) ) {
				...
				$signatures = new Jet_Engine_Listings_Signatures();

				$result['query'] = $signatures->maybe_add_mutable_query_signature(
					$result['query'],
					$result['widget_settings']
				);

				$signature = $signatures->generate_signature( $result['query'] );
		...
		public function posts_template( $query, $settings ) {
			...
			if ( ! empty( $query ) ) {
				...
				$is_slick = jet_engine()->misc_settings->get_settings( 'slider_library' ) === 'slick';
				...
				if ( $carousel_enabled ) {
					...
					$dir                     = $is_rtl ? 'rtl' : 'ltr';
					$slider_suffix           = $is_slick ? '__slider slick-lib' : '__slider swiper-lib swiper';

					printf(
						'<div class="%1$s%4$s" data-slider_options="%2$s" dir="%3$s">',
