		...
		public function get_engine_types() {
			...
			if ( ! empty( $items ) ) {
				foreach ( $items as $item ) {
					...
					$item['labels'] = jet_engine_safe_unserialize_array( $item['labels'], 'post_types_list_labels' );
					$item['args']   = jet_engine_safe_unserialize_array( $item['args'], 'post_types_list_args' );

					$this->engine_types[ $item['slug'] ] = array(
						'slug'   => $item['slug'],
						'id'     => $item['id'],
						'labels' => array(
							'name' => isset( $item['labels']['name'] ) ? $item['labels']['name'] : '',
						),
					);
