...
class Jet_Engine_CPT_Rest_Edit_Post_Type extends Jet_Engine_Base_API_Endpoint {
	...
	public function callback( $request ) {
		...
		if ( ! empty( $this->safe_get( $params, 'general_settings', 'custom_storage' ) ) ) {
			$fields = \Jet_Engine\CPT\Custom_Tables\Manager::instance()->prepare_fields( $params['meta_fields'], $new_slug )['as_columns'];
			$db = \Jet_Engine\CPT\Custom_Tables\Manager::instance()->get_db_instance( $new_slug, $fields );
			$table = $db->table();
