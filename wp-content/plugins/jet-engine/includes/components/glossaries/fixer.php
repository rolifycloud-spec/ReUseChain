...
class Fixer {
	...
	private function is_glossary_broken() {
		...
		foreach ( $glossary as $key => $value ) {
			if ( isset( $array_values[ $key ] ) && ! is_array( jet_engine_safe_unserialize( $value, 'glossary_fixer/is_broken/' . $key ) ) ) {
				return true;
			}
		}
	...
	public function ensure_glossary_structure( $item = array() ) {
		...
		foreach ( $item as $key => $value ) {
			if ( isset( $array_values[ $key ] ) && ! is_array( jet_engine_safe_unserialize( $value, 'glossary_fixer/ensure/' . $key ) ) ) {
				...
				if ( ! is_array( jet_engine_safe_unserialize( $item[ $key ], 'glossary_fixer/repaired/' . $key ) ) ) {
					$item[ $key ] = array();
				}
			}
		}
