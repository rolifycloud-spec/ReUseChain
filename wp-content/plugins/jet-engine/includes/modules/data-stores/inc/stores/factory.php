...
class Factory {
	...
	public function get_post_count( $post_id = null ) {
		...
		$custom_count = apply_filters( 'jet-engine/data-stores/pre-get-post-count', false, $post_id, $this );

		if ( false !== $custom_count ) {
			return $custom_count;
		}
	...
	public function reset_all_post_counts() {
		...
		if ( $this->is_user_store() ) {
			...
			$wpdb->delete(
				$wpdb->usermeta,
				array(
					'meta_key' => $this->count_posts_key . $this->get_slug(),
				),
				array(
					'%s',
				)
