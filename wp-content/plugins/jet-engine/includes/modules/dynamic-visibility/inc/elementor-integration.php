...
class Elementor_Integration extends Condition_Checker {
	...
	private $current_hidden_id = false;
	...
	public function before_element_render( $element ) {
		...
		if ( ! $is_visible ) {
			if ( ! $this->current_hidden_id ) {
				$this->current_hidden_id = $element->get_id();
				add_filter( 'elementor/element/get_child_type', '__return_false', 100 ); // for prevent getting content of inner elements.
			}

			add_filter( 'elementor/frontend/' . $element->get_type() . '/should_render', '__return_false', 100  );
	...
	public function after_element_render( $element ) {
		if ( $this->current_hidden_id && $this->current_hidden_id === $element->get_id() ) {
			remove_filter( 'elementor/element/get_child_type', '__return_false', 100 );
			$this->current_hidden_id = false;
		}
		...
		remove_filter( 'elementor/frontend/' . $element->get_type() . '/should_render', '__return_false', 100 );
