...
class Jet_Engine_Advanced_Date_Field_Data {
	...
	public function update_field( $result, $post_id, $field_name ) {
		...
		$this->update_field_with_value( $update_field, $post_id, $field_name );
		return true;
	}

	public function update_field_with_value( $value, $post_id, $field_name ) {
		if ( ! is_array( $value ) ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( ! empty( $value['dates'] ) ) {
				$value['dates'] = array_values( $value['dates'] );
			}

			update_post_meta( $post_id, $this->config_field_name( $field_name ), json_encode( $value ) );
			$this->add_field_data( $post_id, $field_name, $value );
		}
	}
	...
	public function add_field_data( $post_id, $field_name, $data ) {
		...
		if ( $end_date_string && ! empty( $data['end_time'] ) ) {
