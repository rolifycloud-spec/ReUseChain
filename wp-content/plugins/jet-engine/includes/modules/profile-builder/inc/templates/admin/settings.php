		...
		<div class="cx-vui-panel">
			...
			>
				...
				>
					...
					<cx-vui-select
						label="<?php _e( 'For not authorized users', 'jet-engine' ); ?>"
						description="<?php _e( 'What to do when non-authorized user try to access user page', 'jet-engine' ); ?>"
						:wrapper-css="[ 'equalwidth' ]"
						size="fullwidth"
						:options-list="userPageNotLoggedActions"
						v-model="settings.user_page_not_logged_in_action"
					></cx-vui-select>
					<cx-vui-f-select
						label="<?php _e( 'Template', 'jet-engine' ); ?>"
						description="<?php _e( 'Select template to show as page content for non-authorized user', 'jet-engine' ); ?>"
						...
						size="fullwidth"
						:multiple="false"
						v-if="'template' === settings.user_page_not_logged_in_action"
						ref="user_page_not_logged_in_template"
						v-model="settings.user_page_not_logged_in_template"
					><jet-profile-new-template @on-create="( value ) => { setCreatedTemplate( settings, 'user_page_not_logged_in_template', value ) }"></jet-profile-new-template></cx-vui-f-select>
					<cx-vui-input
						label="<?php _e( 'Redirect URL', 'jet-engine' ); ?>"
						description="<?php _e( 'Set page URL to redirect', 'jet-engine' ); ?>"
						:wrapper-css="[ 'equalwidth' ]"
						size="fullwidth"
						v-if="'page_redirect' === settings.user_page_not_logged_in_action"
						v-model="settings.user_page_not_logged_in_redirect"
					></cx-vui-input>
					<cx-vui-select
						label="<?php _e( 'For users with restricted access', 'jet-engine' ); ?>"
						description="<?php _e( 'What to do when user without access tries to open Single User page', 'jet-engine' ); ?>"
						:wrapper-css="[ 'equalwidth' ]"
						size="fullwidth"
						:options-list="userPageNotAccessibleActions"
						v-model="settings.user_page_not_accessible_action"
					></cx-vui-select>
					<cx-vui-f-select
						label="<?php _e( 'Template', 'jet-engine' ); ?>"
						description="<?php _e( 'Select template to show as page content for user without access', 'jet-engine' ); ?>"
						:wrapper-css="[ 'equalwidth' ]"
						:remote="true"
						:remote-callback="getPosts"
						size="fullwidth"
						:multiple="false"
						v-if="'template' === settings.user_page_not_accessible_action"
						ref="user_page_not_accessible_template"
						v-model="settings.user_page_not_accessible_template"
					>
						<jet-profile-new-template
							@on-create="( value ) => { setCreatedTemplate( settings, 'user_page_not_accessible_template', value ) }"
						></jet-profile-new-template>
					</cx-vui-f-select>
					<cx-vui-input
						label="<?php _e( 'Redirect URL', 'jet-engine' ); ?>"
						...
						:wrapper-css="[ 'equalwidth' ]"
						size="fullwidth"
						v-if="'page_redirect' === settings.user_page_not_accessible_action"
						v-model="settings.user_page_not_accessible_redirect"
					/>
					<div class="cx-vui-inner-panel">
						<cx-vui-repeater
							button-label="<?php _e( '+ Add New Subpage', 'jet-engine' ); ?>"
