...
if ( ! class_exists( 'Jet_Engine_Forms_File_Upload' ) ) {
	...
	class Jet_Engine_Forms_File_Upload {
		...
		/**
		 * Return the MIME map allowed for Legacy Forms uploads.
		 *
		 * The map is derived server-side because a browser-provided file type is
		 * not trustworthy. SVG is intentionally excluded from Legacy Forms: these
		 * files are served directly from uploads and require a dedicated sanitizer.
		 *
		 * @param  array $allowed_mimes Saved field MIME values.
		 * @return array
		 */
		public function get_upload_mimes( $allowed_mimes = array() ) {

			$mimes = get_allowed_mime_types();

			if ( ! empty( $allowed_mimes ) ) {
				$allowed_mimes = array_filter( (array) $allowed_mimes, 'is_string' );

				$mimes = array_filter( $mimes, function( $mime ) use ( $allowed_mimes ) {
					return in_array( $mime, $allowed_mimes, true );
				} );
			}

			return array_filter( $mimes, function( $mime, $extension ) {
				return 'image/svg+xml' !== $mime && ! preg_match( '/(^|\\|)svgz?($|\\|)/i', $extension );
		...
		public function ajax_file_upload() {
			...
			$settings['mime_types'] = ! empty( $field_data['allowed_mimes'] ) ? $field_data['allowed_mimes'] : array();
		...
		public function process_upload( $files = false, $settings = array() ) {
			...
			$settings['mimes'] = $this->get_upload_mimes( $settings['mime_types'] );

			// An explicit field allowlist with no currently permitted types must not
			// fall back to WordPress's global MIME map.
			if ( empty( $settings['mimes'] ) ) {
			...
			foreach ( $files as $file ) {
				...
				$this->ensure_upload_dependencies();

				if ( empty( $file['tmp_name'] ) || empty( $file['name'] ) ) {
					wp_send_json_error( $settings['messages']['upload_mime_types'] );
				}

				$filetype = wp_check_filetype_and_ext( $file['tmp_name'], $file['name'], $settings['mimes'] );

				if ( empty( $filetype['ext'] ) || empty( $filetype['type'] ) ) {
					wp_send_json_error( $settings['messages']['upload_mime_types'] );
				}

				$result[] = $this->upload_file( $file, $insert_attachment, $settings['mimes'] );
		 ...
		 * @return [type] [description]
		 */
		public function upload_file( $file = array(), $insert_attachment = false, $mimes = null ) {

			$result = array();
			$mimes  = null === $mimes ? $this->get_upload_mimes() : $mimes;

			// Keep direct callers from converting an explicit deny-all map into
			// WordPress's global MIME fallback.
			if ( empty( $mimes ) ) {
				$this->errors[] = __( 'File type is not allowed.', 'jet-engine' );
				return $result;
			}
		...
		/**
		 * Load the WordPress file APIs used for server-side type validation.
		 *
		 * @return void
		 */
		private function ensure_upload_dependencies() {
