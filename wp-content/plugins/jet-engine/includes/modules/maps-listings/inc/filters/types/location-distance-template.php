...
$current_location    = ! empty( $args['current_value'] ) ? wp_json_encode( $args['current_value'] ) : '';
$accessibility_label = ! empty( $args['accessibility_label'] ) ? $args['accessibility_label'] : __( 'Location and Distance filter', 'jet-engine' );
?>
<div class="jet-search-filter jsf-location-distance" <?php $this->filter_data_atts( $args ); ?> data-current-location="<?php echo esc_attr( $current_location ); ?>" data-geolocation-placeholder="<?php echo esc_attr( $geolocation_verbose ); ?>">
	<div class="jsf-location-distance__location jsf-show-locate">
		<input
			type="text"
			class="jsf-location-distance__location-input"
		...
		<div class="jsf-location-distance__location-locate jsf-location-distance__location-control" data-geolocation-verbose="<?php echo esc_attr( $geolocation_verbose ); ?>">
			...
			<?php 
				if ( $geolocation_verbose ) {
					printf( '<div class="jsf-location-distance__tooltip">%s</div>', esc_html( $geolocation_verbose ) );
				}
			?>
		</div>
	...
	<select
		class="jsf-location-distance__distance"
		data-default="<?php echo esc_attr( $args['distance_list'][0] ); ?>"
		data-units="<?php echo esc_attr( $args['distance_units'] ); ?>"
		aria-label="<?php echo esc_attr( $accessibility_label ); ?>"
	><?php
