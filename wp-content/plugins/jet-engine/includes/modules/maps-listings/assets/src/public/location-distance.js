			...
			fillAddressDropdown( query ) {
				...
				this.xhr = $.ajax({
					...
					for ( var i = 0; i < response.data.length; i++ ) {
						...
						entryElement.classList.add( 'jsf-location-distance__location-dropdown-item' );
						entryElement.dataset.address = address;
						entryElement.innerText = address;
						
						this.$locationDropdown.append( entryElement );
					}
