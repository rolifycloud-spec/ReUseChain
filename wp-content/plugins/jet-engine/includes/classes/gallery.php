...
if ( ! class_exists( 'Jet_Engine_Img_Gallery' ) ) {
	...
	class Jet_Engine_Img_Gallery {
		...
		public static function slider( $images = array(), $args = array() ) {
			...
			$lib = jet_engine()->misc_settings->get_settings( 'slider_library' );
			$args['css_classes'][] = $lib . '-lib';

			if ( $args['stretch_image'] !== 'yes' ) {
				$args['css_classes'][] = 'fit-image';
				$args['css_classes'][] = sprintf( 'fit-image-%s', $args['stretch_image'] );
			}
			...
			foreach ( $images as $img_data ) {
				...
				$image_classes = array( 'jet-engine-gallery-slider__item-img' );

				if ( $args['stretch_image'] === 'yes' ) {
					$image_classes[] = 'item-image-fullwidth';
				}
		...
		public static function grid( $images = array(), $args = array() ) {
			...
			foreach ( $images as $img_data ) {
				...
				$image_classes = array( 'jet-engine-gallery-grid__item-img' );
