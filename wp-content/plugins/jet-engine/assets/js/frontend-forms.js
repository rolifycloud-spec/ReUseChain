( function( $ ) {
	...
	var JetEngineForms = {
		...
		getCalculatedFieldNumericValue: function( $field ) {

			// Field values can be controlled by form visitors. Only inject numeric literals into formulas.
			var value       = JetEngineForms.getFieldValue( $field ),
			    numericValue = 0,
			    radixValue   = null;
			...
			if ( 'number' === typeof value ) {
				...
				radixValue = value.match( /^([+-]?)(0[xX][0-9a-fA-F]+|0[bB][01]+|0[oO][0-7]+)$/ );

				if ( radixValue ) {
					numericValue = Number( radixValue[2] );
			...
			if ( 0 === numericValue && -Infinity === ( 1 / numericValue ) ) {
				return '-0';
			}
		...
		calculateValue: function( $scope ) {
			...
			formula = formula.replace( regexp, function( match1, match2 ) {
				...
				return JetEngineForms.getCalculatedFieldNumericValue( object );
