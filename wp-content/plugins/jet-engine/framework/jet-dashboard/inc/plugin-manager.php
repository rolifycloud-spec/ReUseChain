...
class Plugin_Manager {
	...
	public function __construct() {
		...
		$this->sys_messages = apply_filters( 'jet_dashboard_plugin_sys_messages', array(
		...
		/**
		 * Need for test update - delete_site_transient( 'update_plugins' );
		 */
		...
		add_filter( 'http_request_args', array( $this, 'allow_unsafe_urls' ), 10, 2 );
	...
	public function activate_plugin_handle( $plugin, $network_wide ) {
		...
		if ( ! is_array( $jet_plugin_list ) ) {
			return false;
		}

		$is_jet_plugin = array_search( $plugin, array_column( $jet_plugin_list, 'slug' ), true );

		if ( false === $is_jet_plugin ) {
			return false;
		}
	...
	public function plugin_action() {
		...
		$plugin  = isset( $data['plugin'] ) ? $this->normalize_plugin_file( $data['plugin'] ) : false;
		$version = isset( $data['version'] ) ? sanitize_text_field( $data['version'] ) : false;

		if ( ! in_array( $action, array( 'activate', 'deactivate', 'install', 'update', 'rollback' ), true ) ) {
			wp_send_json( array(
				'status'  => 'error',
	...
	public function activate_plugin( $plugin_file ) {
		...
		if ( ! $this->is_dashboard_plugin_allowed( $plugin_file, true ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Sorry, you are not allowed to activate this plugin from Jet Dashboard.'
				)
			);
		}
	...
	public function deactivate_plugin( $plugin_file ) {
		...
		if ( ! $this->is_dashboard_plugin_allowed( $plugin_file ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Sorry, you are not allowed to deactivate this plugin from Jet Dashboard.'
				)
			);
		}
	...
	public function install_plugin( $plugin_file, $plugin_url = false ) {
		...
		if ( ! $this->is_dashboard_plugin_allowed( $plugin_file, false, $plugin_url ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Sorry, you are not allowed to install this plugin from Jet Dashboard.'
				)
			);
		}
		...
		if ( ! $package || ! wp_http_validate_url( $package ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Plugin package is not available.',
					'data'    => [],
				)
			);
		}
	...
	public function update_plugin( $plugin_file ) {
		...
		if ( ! $this->is_dashboard_plugin_allowed( $plugin_file ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Sorry, you are not allowed to update this plugin from Jet Dashboard.'
				)
			);
		}
	...
	public function rollback_plugin( $plugin_file = false, $version = false ) {
		...
		$plugin_file = $this->normalize_plugin_file( $plugin_file );
		$version     = $version ? sanitize_text_field( $version ) : false;
		...
		if ( ! $this->is_dashboard_plugin_allowed( $plugin_file ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Sorry, you are not allowed to rollback this plugin from Jet Dashboard.'
				)
			);
		}

		if ( ! $this->is_rollback_version_allowed( $plugin_file, $version ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => 'Requested plugin version is not available for rollback.'
				)
			);
		}
	 ...
	 * @return [type]       [description]
	 */
	public function allow_unsafe_urls( $args, $url = '' ) {

		if ( isset( $_REQUEST['action'] ) && 'jet_dashboard_plugin_action' === $_REQUEST['action'] ) {
			$host = wp_parse_url( $url, PHP_URL_HOST );

			if ( in_array( $host, array( 'api.crocoblock.com', 'account.crocoblock.com' ), true ) ) {
	...
	/**
	 * Normalize a plugin file received from the dashboard UI.
	 *
	 * @param string $plugin_file Plugin file.
	 * @return string|false
	 */
	public function normalize_plugin_file( $plugin_file ) {
	...
	/**
	 * Check whether a plugin can be managed by this dashboard module.
	 *
	 * @param string $plugin_file Plugin file.
	 * @param bool   $allow_wizard Whether to allow Crocoblock Wizard.
	 * @param string $plugin_url Optional package URL.
	 * @return bool
	 */
	public function is_dashboard_plugin_allowed( $plugin_file, $allow_wizard = false, $plugin_url = false ) {

		if ( $allow_wizard && 'crocoblock-wizard/crocoblock-wizard.php' === $plugin_file ) {
			return true;
		}

		if ( $plugin_url && 'crocoblock-wizard/crocoblock-wizard.php' === $plugin_file && 'https://account.crocoblock.com/free-download/crocoblock-wizard.zip' === $plugin_url ) {
			return true;
		}

		$registered_plugins = Dashboard::get_instance()->get_registered_plugins();

		if ( array_key_exists( $plugin_file, $registered_plugins ) ) {
			return true;
		}

		$remote_plugins = $this->get_remote_jet_plugin_list();

		if ( ! is_array( $remote_plugins ) ) {
			return false;
		}

		return in_array( $plugin_file, array_column( $remote_plugins, 'slug' ), true );
	}

	/**
	 * Check whether requested rollback version exists in remote plugin data.
	 *
	 * @param string $plugin_file Plugin file.
	 * @param string $version Version.
	 * @return bool
	 */
	public function is_rollback_version_allowed( $plugin_file, $version ) {

		if ( ! $version || ! preg_match( '/^[0-9]+(?:\.[0-9]+){1,3}(?:[-a-zA-Z0-9.]+)?$/', $version ) ) {
			return false;
		}
		...
		foreach ( $remote_plugins as $plugin_data ) {
			if ( isset( $plugin_data['slug'] ) && $plugin_file === $plugin_data['slug'] ) {
				$versions = isset( $plugin_data['versions'] ) && is_array( $plugin_data['versions'] ) ? $plugin_data['versions'] : array();
