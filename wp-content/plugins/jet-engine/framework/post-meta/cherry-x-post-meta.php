...
if ( ! class_exists( 'Cherry_X_Post_Meta' ) ) {
	...
	class Cherry_X_Post_Meta {
		 ...
		 * @return string
		 */
		public static function _get_repeater_separate_field_key( $repeater_key, $field_key ) {
		 ...
		 * @param $field
		 */
		public static function _delete_repeater_separate_fields( $post_id, $key, $field ) {
			...
			foreach ( $field['fields'] as $repeater_field_key => $repeater_field ) {
				delete_post_meta( $post_id, self::_get_repeater_separate_field_key( $key, $repeater_field_key ) );
			}
		}
		 ...
		 * @param $field
		 */
		public static function _save_repeater_separate_fields( $post_id, $key, $value, $field ) {

			self::_delete_repeater_separate_fields( $post_id, $key, $field );
			...
			foreach ( $value as $repeater_item_value ) {
				...
				foreach ( $repeater_item_value as $repeater_field_key => $repeater_field_value ) {
					add_post_meta(
						$post_id,
						self::_get_repeater_separate_field_key( $key, $repeater_field_key ),
		...
		public function get_repeater_separate_field_key( $repeater_key, $field_key ) {
			return self::_get_repeater_separate_field_key( $repeater_key, $field_key );
		}
		...
		public function save_repeater_separate_fields( $post_id, $key, $value, $field ) {
			self::_save_repeater_separate_fields( $post_id, $key, $value, $field );
		}
