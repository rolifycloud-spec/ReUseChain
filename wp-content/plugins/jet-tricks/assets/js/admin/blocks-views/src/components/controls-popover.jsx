...
const ControlsPopover = ( { children, isOpen, onClose, label, anchor } ) => {
	...
	return (
		<Popover
			...
			offset={ 10 }
		>
			<div className="jet-tricks__popover" style={ { minWidth: '300px', padding: '8px 16px 24px' } }>
				<div className="jet-tricks__popover-header" style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' } }>
					<b>{ label }</b>
					<Button
						className="jet-tricks__popover-close"
						onClick={ onClose }
						icon={ closeIcon } label="Close"
					></Button>
				</div>
				<div className="jet-tricks__popover-content" style={ {
					display: 'flex',
