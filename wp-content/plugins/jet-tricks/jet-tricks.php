...
if ( ! class_exists( 'Jet_Tricks' ) ) {
	...
	class Jet_Tricks {
		...
		/**
		 * Gutenberg blocks views component.
		 *
		 * @since 1.0.0
		 * @access public
		 * @var    Jet_Tricks_Blocks_Views|null
		 */
		public $blocks_views = null;
		...
		public function module_loader() {
			...
			$this->module_loader = new Jet_Tricks_CX_Loader(
				array(
					...
					$this->plugin_path( 'includes/modules/blocks-style-manager/style-manager.php' ),
				)
			);
		}
		...
		public function init() {
			...
			$this->init_blocks_views();

			$this->load_core_files();

			jet_tricks_settings()->init();

			( new Jet_Tricks_Blocks_Particles_Extension() )->init();
			( new Jet_Tricks_Blocks_Tooltip_Extension() )->init();
			( new Jet_Tricks_Blocks_Satellite_Extension() )->init();
			( new Jet_Tricks_Blocks_Parallax_Extension() )->init();
			( new Jet_Tricks_Blocks_Sticky_Column_Extension() )->init();
			( new Jet_Tricks_Blocks_Scroll_Reveal_Extension() )->init();
			( new Jet_Tricks_Blocks_Extensions_Render() )->init();

			if ( $this->has_elementor() ) {
				$this->load_elementor_integration_files();
		...
		/**
		 * Initialize Gutenberg blocks views.
		 *
		 * @return void
		 */
		public function init_blocks_views() {

			require $this->plugin_path( 'includes/components/blocks-views/manager.php' );

			$this->blocks_views = new Jet_Tricks_Blocks_Views();
		}
		...
		/**
		 * Core files (settings, blocks extensions, assets, REST) — no Elementor dependency.
		 *
		 * @return void
		 */
		public function load_core_files() {
			...
			require $this->plugin_path( 'includes/components/blocks-views/extensions/particles.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/tooltip.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/satellite.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/parallax.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/sticky-column.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/scroll-reveal.php' );
			require $this->plugin_path( 'includes/components/blocks-views/extensions/render.php' );
		...
		/**
		 * Elementor-only PHP (extensions + integration).
		 *
		 * @return void
		 */
		public function load_elementor_integration_files() {
			require $this->plugin_path( 'includes/components/elementor-views/extensions/scroll-reveal/class-jet-tricks-elementor-scroll-reveal.php' );
			require $this->plugin_path( 'includes/components/elementor-views/extensions/element-section-extension.php' );
			require $this->plugin_path( 'includes/components/elementor-views/extensions/element-column-extension.php' );
			require $this->plugin_path( 'includes/components/elementor-views/extensions/element-widget-extension.php' );
			require $this->plugin_path( 'includes/integration.php' );
		}

		/**
		 * Load all PHP files (core + Elementor). Use when Elementor is present.
		 *
		 * @return void
		 */
		 ...
		 */
		/**
		 * Elementor instance or null if not loaded.
		 *
