...
if ( ! class_exists( 'Jet_Tricks_Settings' ) ) {
	...
	class Jet_Tricks_Settings {
		...
		public function init() {
			...
			foreach ( glob( jet_tricks()->plugin_path( 'includes/components/elementor-views/widgets/' ) . '*.php' ) as $file ) {
		...
		public function get_frontend_config_data() {
			...
			$avaliable_extensions = [
				...
				[
					'label' => esc_html__( 'Scroll Reveal Extension', 'jet-tricks' ),
					'value' => 'widget_scroll_reveal',
				],
				[
			...
			$active_extensions = $this->get_avaliable_extensions();
		...
		/**
		 * Returns available extensions merged with defaults.
		 *
		 * @return array
		 */
		public function get_avaliable_extensions() {
			return wp_parse_args(
				$this->get( 'avaliable_extensions', array() ),
				$this->default_avaliable_extensions
			);
		}
