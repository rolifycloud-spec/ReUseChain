...
if ( ! class_exists( 'Jet_Reviews_Tools' ) ) {
	...
	class Jet_Reviews_Tools {
		...
		/**
		 * Decode stored HTML entities for plain-text fields.
		 *
		 * @param  string $value Raw field value.
		 * @return string
		 */
		public function decode_plain_text( $value = '' ) {
		...
		/**
		 * Decode stored editor HTML to plain text for admin textarea fields.
		 *
		 * @param  string $value Raw field value.
		 * @return string
		 */
		public function decode_editor_text( $value = '' ) {
			...
			$value = $this->decode_plain_text( $value );

			return strip_tags( $value );
		}

		/**
		 * Encode admin textarea content back to canonical stored HTML.
		 *
		 * @param  string  $value     Raw field value.
		 * @param  boolean $multiline Whether line breaks should be preserved.
		 * @return string
		 */
		public function encode_editor_text( $value = '', $multiline = false ) {
			...
			$value = preg_replace_callback( '/ {2,}/', function( $matches ) {
		...
		public function render_structured_data( $data ) {
			$data = wp_parse_args( $data, array(
				...
				'item_image'    => '',
				'item_desc'     => '',
				'item_type'     => $this->get_default_structure_data_type(),
				'item_rating'   => '',
				'review_date'   => '',
				'review_author' => '',
			) );
			...
			$sdata = array(
				...
				'@type' => 'Review',
				'itemReviewed' => array(
					'@type' => $this->get_valid_structure_data_type( $data['item_type'] ),
					'name' => $data['item_name'],
				),
				'reviewRating' => array(
					'@type' => 'Rating',
					'ratingValue' => $this->normalize_structure_data_rating( $data['item_rating'] ),
					'bestRating' => '5',
					'worstRating' => '1',
				),
			...
			if ( ! empty( $data['item_desc'] ) ) {
				$sdata['itemReviewed']['description'] = $data['item_desc'];
			}

			$json_data = wp_json_encode( $sdata, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT );

			printf( '<script type="application/ld+json">%s</script>', $json_data ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON is safely encoded for a script element above.
		...
		public function get_structure_data_types() {
			...
			return apply_filters( 'jet-reviews/structure-data/types', [
				[
					'label' => 'Google-supported review snippet types',
					'options' => [
						[ 'label' => 'Book', 'value' => 'Book' ],
						[ 'label' => 'Course', 'value' => 'Course' ],
						[ 'label' => 'CreativeWorkSeason', 'value' => 'CreativeWorkSeason' ],
						[ 'label' => 'CreativeWorkSeries', 'value' => 'CreativeWorkSeries' ],
						[ 'label' => 'Episode', 'value' => 'Episode' ],
						[ 'label' => 'Event', 'value' => 'Event' ],
						[ 'label' => 'Game', 'value' => 'Game' ],
						[ 'label' => 'HowTo', 'value' => 'HowTo' ],
						[ 'label' => 'LocalBusiness', 'value' => 'LocalBusiness' ],
						[ 'label' => 'MediaObject', 'value' => 'MediaObject' ],
				...
				[
					'label' => 'Supported subtype options',
					'options' => [
		 ...
		 * @return string[]
		 */
		public function get_structure_data_type_values() {

			return array_keys( $this->get_structure_data_type_options() );
		}

		/**
		 * @return string[]
		 */
		public function get_structure_data_type_options() {

			$options = [];

			foreach ( $this->get_structure_data_types() as $group ) {
				...
				if ( isset( $group['options'] ) && is_array( $group['options'] ) ) {
					foreach ( $group['options'] as $option ) {
						...
						$options[ $option['value'] ] = isset( $option['label'] ) ? $option['label'] : $option['value'];
					}
				} elseif ( ! empty( $group['value'] ) ) {
					$options[ $group['value'] ] = isset( $group['label'] ) ? $group['label'] : $group['value'];
				}
			}
		 ...
		 * @return string
		 */
		public function get_default_structure_data_type() {
		 ...
		 * @return string
		 */
		public function get_valid_structure_data_type( $type = '' ) {

			$type   = is_scalar( $type ) ? (string) $type : '';
			$values = $this->get_structure_data_type_values();

			if ( in_array( $type, $values, true ) ) {
				return $type;
			}

			$default = $this->get_default_structure_data_type();

			if ( in_array( $default, $values, true ) ) {
				return $default;
			}

			return ! empty( $values ) ? reset( $values ) : $default;
		}
		 ...
		 * @return string
		 */
		public function normalize_structure_data_rating( $rating, $min = 1, $max = 5 ) {

			$rating = is_numeric( $rating ) ? (float) $rating : $min;
			$rating = min( $max, max( $min, $rating ) );
